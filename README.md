...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_opinions \
  -H 'Content-Type: application/json' \
  -d '{"query":"Air France v. Saks"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_opinions`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
